...
\title{PAM Reference Book\\[0.4em]\normalsize for PAM v0.9.23}
...
\emph{PAM v0.9.20 · fountain2pam v0.9.13} \emph{Co-authored by David
...
\texttt{set\_background} & scene-level & 0.9.20 & n/a \\
\end{longtable}
...
\item
  ...
  prompts file)
\item
  \textbf{Background:} \texttt{set\_background} (instant scene
  background color change; v0.9.20)
\end{itemize}
...
\section{Version history}\label{version-history}\index{version history}
...
\bottomrule\noalign{}
\endlastfoot
0.9.23 & Backburner items 10--11, 18, and item-6 Option C.
\textbf{2.5D depth system} (items 10--11): optional
\texttt{"depth"}/\texttt{"rescale"} on cast and props entries;
depth-banded z-ordering with dynamic mid-move reordering;
ground-contact-preserving perspective rescale; depth-animated
\texttt{walk\_to}/\texttt{run\_to}/\texttt{trot\_to}; scene-level
\texttt{depth\_config} constant. \textbf{Per-character SFX}
(item 18): cast-level \texttt{sfx} block; three-tier inline > cast >
scene resolution; \texttt{sfx\_gain}/\texttt{sfx\_delay}/
\texttt{sfx\_trigger}. \textbf{Structured warning registry}
(item 6 Option C): \texttt{PAM\_WARNINGS\_JSON} dumps
(array\_index, action\_type, message) JSON at render completion.
See \S13. \\
0.9.22 & Backburner items 4 and 19. (4) \textbf{Rotation-aware
\texttt{morph\_to}}: figures maintain the placement invariant
$\mathrm{world} = R(\theta)\,p_{\mathrm{scaled}} + \mathrm{offset}$;
\texttt{rotate} updates both terms via \texttt{note\_rotation()}
(offset conjugated about the pivot), so \texttt{morph\_to} /
\texttt{set\_pose} render new poses in the rotated, relocated frame
instead of snapping back to pose space. Rotation sequences about
different pivots compose exactly. Companion \texttt{act\_rotate}
patch adds \textbf{named pivots} (\texttt{"pivot": "bevers"}) and
live-dot pivot derivation. (19) \textbf{Cast merge-on-redefine}: a
mid-scene \texttt{cast} block inherits unstated keys (scale, style,
tics, gloves\ldots) from the prior entry instead of silently
resetting them; \texttt{style} merges one level deep;
\texttt{figure\_type} / \texttt{pose} / \texttt{offset} always take
the new value; state a key explicitly (e.g.\ \texttt{"scale": null})
to reset it. Also in this version: the \texttt{punch} character
interaction (jab + target recoil + stagger, §12.4) and the
\texttt{dog\_sitting} pose with sloped-spine support in
\texttt{dog\_side\_pose()} (§12.5). See §12. \\
0.9.21 & Backburner items 1--3. (1) \textbf{Cast+props sidecar}: the
player loads an optional shared declarations file (\texttt{PAM\_SIDECAR}
env var, or \texttt{pam\_sidecar.json} next to \texttt{PAM\_SCRIPT})
carrying \texttt{cast} / \texttt{faces} / \texttt{props} /
\texttt{scene\_props} blocks; scene definitions take priority on
collision; silent no-op if absent. (2) \textbf{\texttt{end\_scene}}
action: one-step teardown of all live props (dual-registered dog
characters excepted), scene objects, and persistent bubbles, plus a
background reset to the player default; optional \texttt{rt} for
animated removal (default instant). (3) \textbf{Cast-level glove/shoe
defaults}: \texttt{glove\_color}, \texttt{glove\_stroke},
\texttt{glove\_size}, \texttt{shoe\_color}, \texttt{shoe\_stroke},
\texttt{shoe\_size} keys on a cast entry auto-apply
\texttt{attach\_gloves} / \texttt{attach\_shoes} at \texttt{fade\_in};
explicit attach actions in the scene body still override. Also in this
version: backburner item 5 (rotation-aware \texttt{say} anchoring ---
bubbles now anchor to the live head-dot position, closed by the fix
already present in \texttt{figure.py}; documented in §11.4), item 6
(\texttt{PAM\_LOG} / \texttt{PAM\_WARNINGS\_ONLY} console-log tees,
§11.5), item 7 (in-scene audio cues: an \texttt{audio} defaults block
plus per-step \texttt{sound} keys, baked via \texttt{add\_sound()} at
dispatch time, with transparent \texttt{.m4a} conversion; §11.7), and
item 8 (\texttt{pam\_merge\_audio.py} post-render
soundtrack merge CLI, §11.6). Fixes and tooling (§11.8): item 14
(\texttt{walk\_to}/\texttt{run\_to} on a dog inside \texttt{parallel}
no longer crashes --- reroutes to a trot with a notice), item 15
(\texttt{\_tag\_dog\_group()} helper deduplicates dog dual-registration
tagging), and item 9 (\texttt{pam\_rag\_index.py} /
\texttt{pam\_rag\_query.py} local manual search). See §11 of
the \texttt{pam\_player.py} Reference for full syntax. \\
0.9.20 & \texttt{set\_background} scene-level action. Instantly changes
the Manim camera background color to any hex value; takes effect at the
current frame with no animation. Intended for location-change signaling
(e.g.\ Venus vs.\ Earth scenes). Implemented as a four-line handler in
\texttt{pam\_player.py}'s top-level dispatch, just before
\texttt{remove\_scene\_object}. No effect on characters, props, or the
camera position. See §10.5 for full syntax and usage. \\\
0.9.19 & Sticky z-order for characters and props.
\texttt{fade\_in} and \texttt{cast} block entries now accept
\texttt{z\_index}; the value is stored in \texttt{cast[name]["z\_index"]}
and re-applied after every internal \texttt{bring\_to\_front} call so
that \texttt{focus}, \texttt{focus\_reset}, and \texttt{spawn\_prop}
cannot clobber a declared occlusion hierarchy.
New private helper \texttt{\_reassert\_z\_indices()} fires after
\texttt{scene\_objects} background-prop placement, both
\texttt{spawn\_prop} paths, and \texttt{\_repair\_face\_overlays}.
\texttt{spawn\_prop} \texttt{z\_index} / \texttt{z\_offset} behavior
unchanged; \texttt{\_reassert\_z\_indices} is now also called after those
paths. See Prop Properties Reference §5.1 for the full z-order system
and recommended layer-range convention, and
\hyperref[fade-in-z-index]{pam\_player §5.4} for
\texttt{fade\_in} syntax. \\
...
\section{1. Overview}\label{overview}\index{PAMPlayer}\index{MovingCameraScene@\texttt{MovingCameraScene}}
...
\subsection{\texorpdfstring{5.3 \texttt{spawn\_prop} /\index{spawn\_prop@\texttt{spawn\_prop}}\index{spawn\_prop@\texttt{spawn\_prop}!z\_index}\index{spawn\_prop@\texttt{spawn\_prop}!bring\_to\_front}
...
\textbf{z\_index and bring\_to\_front (v0.9.19).}
\texttt{spawn\_prop} accepts two optional stacking keys:

\begin{itemize}
\tightlist
\item
  \texttt{z\_index} (or alias \texttt{z\_offset}) --- sets Manim's
  native z-layer on the prop mobject. Higher values render in front.
\item
  \texttt{bring\_to\_front:\ true} --- calls Manim's
  \texttt{bring\_to\_front()} immediately after spawn, placing the prop
  above all current scene mobjects regardless of z-index.
\end{itemize}

After either operation, \texttt{\_reassert\_z\_indices()} re-applies all
character \texttt{z\_index} values so that spawning a foreground prop
does not accidentally push characters behind their declared layer.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Foreground table that occludes characters (z\_index > character z\_index)}
\NormalTok{\{}
\NormalTok{  "action": "spawn\_prop",}
\NormalTok{  "prop": "reception\_desk",}
\NormalTok{  "type": "desk",}
\NormalTok{  "x": 1.0,}
\NormalTok{  "z\_index": 7}
\NormalTok{\}}

\CommentTok{// Background chair (z\_index < character z\_index)}
\NormalTok{\{}
\NormalTok{  "action": "spawn\_prop",}
\NormalTok{  "prop": "back\_chair",}
\NormalTok{  "type": "chair",}
\NormalTok{  "x": -1.5,}
\NormalTok{  "z\_index": 2}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
parent-attached. See Prop Properties Reference §5.1 for the full
z-order system including the recommended layer-range convention.

\subsection{\texorpdfstring{5.4 \texttt{fade\_in} --- figure\index{fade\_in@\texttt{fade\_in}}\index{HumanGraph@\texttt{HumanGraph}!construction}\index{AlienGraph@\texttt{AlienGraph}!construction}
...
\subsubsection{\texttt{z\_index} on \texttt{fade\_in} (v0.9.19)}\label{fade-in-z-index}\index{fade\_in@\texttt{fade\_in}!z\_index}

A \texttt{z\_index} key on the \texttt{fade\_in} step (or on the
corresponding \texttt{cast} entry in multi-character scenes) sets the
character's Manim layer and makes it \emph{sticky}: the value is stored
in \texttt{cast[name]["z\_index"]} and re-applied automatically after
every internal \texttt{bring\_to\_front} call (\texttt{focus},
\texttt{focus\_reset}, \texttt{spawn\_prop} with
\texttt{bring\_to\_front:\ true}).

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Inline on fade\_in}
\NormalTok{\{}
\NormalTok{  "action": "fade\_in",}
\NormalTok{  "who": "chava",}
\NormalTok{  "offset": [0, 0, 0],}
\NormalTok{  "pose": "standing",}
\NormalTok{  "z\_index": 3}
\NormalTok{\}}

\CommentTok{// Or in the cast block}
\NormalTok{"cast": \{}
\NormalTok{  "chava": \{}
\NormalTok{    "figure\_type": "human",}
\NormalTok{    "offset": [0, 0, 0],}
\NormalTok{    "z\_index": 3}
\NormalTok{  \}}
\NormalTok{\}}
...
Combined with \texttt{z\_index} values on \texttt{spawn\_prop}, this
lets you declare a full occlusion hierarchy. For example, to place Chava
behind a table but in front of a chair:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\{"action": "spawn\_prop", "prop": "chair", "type": "chair",}
\NormalTok{ "x": -0.5, "z\_index": 1\},}

\NormalTok{\{"action": "fade\_in", "who": "chava", "offset": [0, 0, 0], "z\_index": 3\},}

\NormalTok{\{"action": "spawn\_prop", "prop": "table", "type": "table",}
\NormalTok{ "x": 0.5, "z\_index": 5\}}
\end{Highlighting}
\end{Shaded}

The resulting render order (back to front): chair → Chava → table.
\texttt{focus}, \texttt{focus\_reset}, and any subsequent
\texttt{spawn\_prop} calls will not disturb this order.
...
\subsection{\texorpdfstring{5.8 \texttt{\_subscene\_marker} --- camera\index{\_subscene\_marker@\texttt{\_subscene\_marker}}
reframe
...
\item
  Calls \texttt{\_apply\_camera} immediately for instant reframes
  (\texttt{static}, \texttt{zoom}, \texttt{pan-up}, \texttt{pan-down},
  \texttt{descend}, \texttt{push-into}), or stores
  \texttt{\_camera\_anim(...)} in \texttt{\_pending\_camera} for
  deferred moves (\texttt{push}, \texttt{pull}, \texttt{pan-follow},
  \texttt{drift}) which fire concurrently with the next \texttt{say}.
  See §7.3 for the full immediate vs.\ deferred table.
\end{enumerate}
...
\section{7. The camera system}\label{the-camera-system}\index{camera system}\index{PAMPlayer!camera system}
...
\subsection{\texorpdfstring{7.2 The \texttt{\_FRAMING\_CAMERA}\index{\_FRAMING\_CAMERA@\texttt{\_FRAMING\_CAMERA}}\index{framing|see{\texttt{\_FRAMING\_CAMERA}}}
...
\subsection{\texorpdfstring{7.3 The \texttt{\_MOVE\_RT}\index{\_MOVE\_RT@\texttt{\_MOVE\_RT}}\index{camera moves!move types}
dict}{7.3 The \_MOVE\_RT dict}}\label{the-_move_rt-dict}

Maps move names to default animation durations (seconds). \texttt{None}
entries own their own \texttt{scene.play()} call and are intercepted
before reaching \texttt{\_apply\_camera}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\_MOVE\_RT: }\BuiltInTok{dict}\NormalTok{[}\BuiltInTok{str}\NormalTok{, }\BuiltInTok{float}\NormalTok{] }\OperatorTok{=}\NormalTok{ \{}
    \StringTok{"static"}\NormalTok{:     }\FloatTok{0.0}\NormalTok{,    }\CommentTok{\# instant — no scene.play() call}
    \StringTok{"push"}\NormalTok{:       }\FloatTok{0.8}\NormalTok{,    }\CommentTok{\# deferred — fires with next say}
    \StringTok{"pull"}\NormalTok{:       }\FloatTok{0.8}\NormalTok{,    }\CommentTok{\# deferred — fires with next say}
    \StringTok{"pan{-}follow"}\NormalTok{: }\FloatTok{0.5}\NormalTok{,    }\CommentTok{\# deferred}
    \StringTok{"drift"}\NormalTok{:      }\FloatTok{1.5}\NormalTok{,    }\CommentTok{\# deferred}
    \StringTok{"pan{-}up"}\NormalTok{:     }\FloatTok{2.5}\NormalTok{,    }\CommentTok{\# immediate — \_execute\_pan\_up owns play()}
    \StringTok{"pan{-}down"}\NormalTok{:   }\FloatTok{2.5}\NormalTok{,    }\CommentTok{\# immediate — \_execute\_pan\_down owns play()}
    \StringTok{"descend"}\NormalTok{:    }\VariableTok{None}\NormalTok{,   }\CommentTok{\# immediate — rt from meta}
    \StringTok{"push{-}into"}\NormalTok{:  }\VariableTok{None}\NormalTok{,   }\CommentTok{\# immediate — rt from meta}
    \StringTok{"zoom"}\NormalTok{:       }\FloatTok{1.2}\NormalTok{,    }\CommentTok{\# v0.9.19 — immediate, rt overridable}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

\textbf{Immediate vs.\ deferred moves.} This is the key dispatch
distinction in the \texttt{\_subscene\_marker} handler:

\begin{itemize}
\tightlist
\item
  \textbf{Immediate} moves (\texttt{static}, \texttt{pan-up},
  \texttt{pan-down}, \texttt{descend}, \texttt{push-into},
  \texttt{zoom}) call \texttt{scene.play()} (or reposition the frame
  directly) at the moment the marker is encountered.  Scene time is
  consumed before the next action begins.
\item
  \textbf{Deferred} moves (\texttt{push}, \texttt{pull},
  \texttt{pan-follow}, \texttt{drift}) are stored in
  \texttt{\_pending\_camera} and only fire when the next \texttt{say} or
  \texttt{prop\_say} consumes them — the camera glides \emph{during} the
  bubble fade-in.  If no \texttt{say} follows before the next marker,
  the pending move is silently discarded.
\end{itemize}

\begin{longtable}[]{@{}llll@{}}
\toprule\noalign{}
Move & Duration & Dispatch & Notes \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\texttt{static}     & 0 s    & immediate & Hard cut; no \texttt{scene.play()} \\
\texttt{push}       & 0.8 s  & deferred  & Zoom in; needs a following \texttt{say} \\
\texttt{pull}       & 0.8 s  & deferred  & Zoom out; needs a following \texttt{say} \\
\texttt{pan-follow} & 0.5 s  & deferred  & Horizontal recentre \\
\texttt{drift}      & 1.5 s  & deferred  & Slow pan; needs a following \texttt{say} \\
\texttt{pan-up}     & 2.5 s  & immediate & Tilt up + optional shear (§7.8) \\
\texttt{pan-down}   & 2.5 s  & immediate & Tilt down \\
\texttt{descend}    & meta \texttt{rt} & immediate & Long vertical travel \\
\texttt{push-into}  & meta \texttt{rt} & immediate & Zoom to prop sign + flash \\
\texttt{zoom}       & 1.2 s  & immediate & Smooth reframe, \texttt{rt} overridable (v0.9.19) \\
\end{longtable}

\subsection{\texorpdfstring{7.4 \texttt{\_apply\_camera} --- instant and\index{\_apply\_camera@\texttt{\_apply\_camera}}\index{rt\_override@\texttt{rt\_override}}
animated
reframes}{7.4 \_apply\_camera --- instant and animated reframes}}\label{apply_camera-instant-and-animated-reframes}

The authoritative camera reposition function. As of v0.9.19 it accepts
an optional \texttt{rt\_override} that bypasses the \texttt{\_MOVE\_RT}
table --- used by the \texttt{zoom} move so the screenplay can specify
an arbitrary duration via \texttt{"rt"} on the \texttt{\_shot\_meta}:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ \_apply\_camera(meta, scene, char\_x\_positions,}
\NormalTok{                   rt\_override}\OperatorTok{=}\VariableTok{None}\NormalTok{):}
...
\NormalTok{    target\_w, target\_y }\OperatorTok{=}\NormalTok{ \_FRAMING\_CAMERA[framing]}
    \CommentTok{\# rt\_override (v0.9.19) takes precedence over \_MOVE\_RT}
    \ControlFlowTok{if}\NormalTok{ rt\_override }\KeywordTok{is} \KeywordTok{not} \VariableTok{None}\NormalTok{:}
\NormalTok{        rt }\OperatorTok{=} \BuiltInTok{float}\NormalTok{(rt\_override)}
    \ControlFlowTok{else}\NormalTok{:}
\NormalTok{        rt }\OperatorTok{=}\NormalTok{ \_MOVE\_RT.get(move, }\FloatTok{0.0}\NormalTok{) }\KeywordTok{or} \FloatTok{0.0}
...
\subsection{\texorpdfstring{7.5 \texttt{\_camera\_anim} and\index{\_camera\_anim@\texttt{\_camera\_anim}}\index{\_pending\_camera@\texttt{\_pending\_camera}}\index{camera moves!deferred}
...
\subsection{\texorpdfstring{7.6 The \texttt{\_shot\_meta}\index{\_shot\_meta@\texttt{\_shot\_meta}}\index{\_subscene\_marker@\texttt{\_subscene\_marker}!shot\_meta keys}
...
\texttt{drift} / \texttt{zoom} & \texttt{static} \\
...
\subsection{\texorpdfstring{7.8 \texttt{pan-up} / \texttt{pan-down} and\index{pan-up@\texttt{pan-up}}\index{pan-down@\texttt{pan-down}}\index{camera moves!pan-up}\index{camera moves!pan-down}\index{shear!camera keystoning}
the tilt
...
\subsection{\texorpdfstring{7.9 The \texttt{insert} framing for prop\index{insert framing@\texttt{insert} framing}\index{framing!insert}
...
\subsection{\texorpdfstring{7.10 The \texttt{zoom}\index{zoom@\texttt{zoom} (camera move)}\index{camera moves!zoom}\index{camera moves!immediate dispatch}
move}{7.10 The zoom move}}\label{the-zoom-move}

\texttt{zoom} (v0.9.19) is a smooth animated reframe that fires
\textbf{immediately} at the \texttt{\_subscene\_marker}, consuming scene
time before the next action. It is the correct choice when you want a
visible camera transition between two framings regardless of whether
dialogue follows.

\subsubsection{How it differs from \texttt{push}/\texttt{pull}/\texttt{drift}}\label{zoom-vs-deferred}

\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
Property & \texttt{zoom} & \texttt{push} / \texttt{pull} / \texttt{drift} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Dispatch & Immediate & Deferred \\
Needs following \texttt{say}? & No & Yes (silently discarded otherwise) \\
Default duration & 1.2 s & 0.8 s / 0.8 s / 1.5 s \\
\texttt{rt} override & Yes & No \\
Direction & Any framing change & push = zoom in; pull = zoom out \\
\end{longtable}

\subsubsection{JSON syntax}\label{zoom-json-syntax}

Minimal (default 1.2 s):

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{\{}
\NormalTok{  }\DataTypeTok{"\_subscene\_marker"}\NormalTok{: }\StringTok{"scene\_03\_ss02b"}\NormalTok{,}
\NormalTok{  }\DataTypeTok{"\_shot\_meta"}\NormalTok{: \{}
\NormalTok{    }\DataTypeTok{"framing"}\NormalTok{:  }\StringTok{"medium-close"}\NormalTok{,}
\NormalTok{    }\DataTypeTok{"subject"}\NormalTok{:  }\StringTok{"petroplast\_building"}\NormalTok{,}
\NormalTok{    }\DataTypeTok{"move"}\NormalTok{:     }\StringTok{"zoom"}
\NormalTok{  \}}
\FunctionTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{    }\DataTypeTok{"framing"}\NormalTok{:  }\StringTok{"medium-close"}\NormalTok{,}
\NormalTok{    }\DataTypeTok{"subject"}\NormalTok{:  }\StringTok{"petroplast\_building"}\NormalTok{,}
\NormalTok{    }\DataTypeTok{"move"}\NormalTok{:     }\StringTok{"zoom"}\NormalTok{,}
\NormalTok{    }\DataTypeTok{"rt"}\NormalTok{:       }\FloatTok{1.5}
\NormalTok{  \}}
\FunctionTok{\}}
\end{Highlighting}
\end{Shaded}

\subsubsection{Combining \texttt{zoom} with \texttt{pan-up}}\label{zoom-with-pan-up}

\texttt{pan-up} snaps the camera to medium-close framing at its own
start, so a \texttt{zoom} immediately before it creates a smooth
approach before the tilt begins. The pattern is a bridging marker with
no actions of its own:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// ss01: wide establishing shot}
\FunctionTok{\{}\DataTypeTok{"\_subscene\_marker"}\NormalTok{: }\StringTok{"scene\_ss01"}\NormalTok{, }\DataTypeTok{"\_shot\_meta"}\NormalTok{: \{}\DataTypeTok{"framing"}\NormalTok{: }\StringTok{"wide"}\NormalTok{,}
\NormalTok{  }\DataTypeTok{"subject"}\NormalTok{: }\StringTok{"ensemble"}\NormalTok{, }\DataTypeTok{"move"}\NormalTok{: }\StringTok{"static"}\NormalTok{\}\}}

\CommentTok{// ss02b: bridge marker — zoom in, no actions}
\FunctionTok{\{}\DataTypeTok{"\_subscene\_marker"}\NormalTok{: }\StringTok{"scene\_ss02b"}\NormalTok{, }\DataTypeTok{"\_shot\_meta"}\NormalTok{: \{}\DataTypeTok{"framing"}\NormalTok{: }\StringTok{"medium-close"}\NormalTok{,}
\NormalTok{  }\DataTypeTok{"subject"}\NormalTok{: }\StringTok{"petroplast\_building"}\NormalTok{, }\DataTypeTok{"move"}\NormalTok{: }\StringTok{"zoom"}\NormalTok{, }\DataTypeTok{"rt"}\NormalTok{: }\FloatTok{1.2}\NormalTok{\}\}}

\CommentTok{// ss03: pan-up tilt shot}
\FunctionTok{\{}\DataTypeTok{"\_subscene\_marker"}\NormalTok{: }\StringTok{"scene\_ss03"}\NormalTok{, }\DataTypeTok{"\_shot\_meta"}\NormalTok{: \{}\DataTypeTok{"framing"}\NormalTok{: }\StringTok{"medium-close"}\NormalTok{,}
\NormalTok{  }\DataTypeTok{"subject"}\NormalTok{: }\StringTok{"petroplast\_building"}\NormalTok{, }\DataTypeTok{"companions"}\NormalTok{: [}\StringTok{"petroplast\_door"}\NormalTok{],}
\NormalTok{  }\DataTypeTok{"char\_subject"}\NormalTok{: }\StringTok{"chava"}\NormalTok{, }\DataTypeTok{"move"}\NormalTok{: }\StringTok{"pan-up"}\NormalTok{,}
\NormalTok{  }\DataTypeTok{"rt"}\NormalTok{: }\FloatTok{3.0}\NormalTok{, }\DataTypeTok{"rt\_return"}\NormalTok{: }\FloatTok{2.0}\NormalTok{, }\DataTypeTok{"hold"}\NormalTok{: }\FloatTok{1.5}\NormalTok{, }\DataTypeTok{"shear"}\NormalTok{: }\FloatTok{0.5}\NormalTok{\}\}}

\CommentTok{// ss04: zoom back to wide}
\FunctionTok{\{}\DataTypeTok{"\_subscene\_marker"}\NormalTok{: }\StringTok{"scene\_ss04"}\NormalTok{, }\DataTypeTok{"\_shot\_meta"}\NormalTok{: \{}\DataTypeTok{"framing"}\NormalTok{: }\StringTok{"wide"}\NormalTok{,}
\NormalTok{  }\DataTypeTok{"subject"}\NormalTok{: }\StringTok{"ensemble"}\NormalTok{, }\DataTypeTok{"move"}\NormalTok{: }\StringTok{"zoom"}\NormalTok{, }\DataTypeTok{"rt"}\NormalTok{: }\FloatTok{1.2}\NormalTok{\}\}}
\end{Highlighting}
\end{Shaded}

The result is: \emph{wide (static)} $\to$ \emph{zoom in 1.2 s} $\to$
\emph{pan-up tilt 3.0 s} $\to$ \emph{zoom out 1.2 s} $\to$ \emph{wide
holds}.

\subsubsection{Gotcha: \texttt{zoom} consumes scene time}\label{zoom-consumes-time}

Because \texttt{zoom} fires immediately, it adds its \texttt{rt} to the
scene timeline. A \texttt{"rt": 1.5} zoom between two actions adds 1.5 s
of camera-only screen time. If the shot needs to feel
cut-like but still animated, \texttt{"rt": 0.4} gives a quick punch-in
that reads as deliberate rather than a jump cut.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{8. The bubble registry}\label{the-bubble-registry}\index{bubble registry}\index{\_persistent\_bubbles@\texttt{\_persistent\_bubbles}}
...
\subsection{\texorpdfstring{8.4 The \texttt{focus} / \texttt{focus\_reset}\index{focus@\texttt{focus}}\index{focus\_reset@\texttt{focus\_reset}}\index{\_repair\_face\_overlays@\texttt{\_repair\_face\_overlays}}
...
\section{10. Runtime additions in the attached v0.9.14--v0.9.20
...
\subsection{\texorpdfstring{10.5 \texttt{set\_background} --- instant
background color
change}{10.5 set\_background --- instant background color change}}\label{set-background}\index{set\_background@\texttt{set\_background}}\index{background color}

Added in v0.9.20. \texttt{set\_background} is a scene-level action that
immediately sets \texttt{self.camera.background\_color} to a given hex
color string. It fires at the current animation frame with no transition
and does not affect any character, prop, z-order, or camera position.

\subsubsection{Motivation}\label{set-background-motivation}

Prior to v0.9.20, the only way to change the background color
mid-screenplay was to include a \texttt{bg\_color} key inside a
\texttt{\_subscene\_marker} / \texttt{\_shot\_meta} block, which tied
the color change to a camera move. That mechanism works well for
fly-in sequences but is awkward when no camera move is wanted --- for
example at a plain scene cut between locations. \texttt{set\_background}
decouples the color change from any camera action.

A typical use case is using background color as a \emph{location
signal}: deep blue (\texttt{\#1a3a6a}) for Venus interiors, black
(\texttt{\#000000}) for Earth locations. Placing a
\texttt{set\_background} block as the very first action of each scene
(before \texttt{cast}, \texttt{fade\_in}, etc.) ensures the correct
background is established before any character appears.

\subsubsection{JSON syntax}\label{set-background-syntax}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{"action"} & string & yes & must be \texttt{"set\_background"} \\
\texttt{"color"} & hex string & yes & e.g.\ \texttt{"\#1a3a6a"}.
Passed directly to \texttt{self.camera.background\_color}; any
Manim-legal hex string is accepted. \\
\end{longtable}

There are no other sub-keys. The action is not animatable (no
\texttt{rt} parameter) and cannot appear inside a \texttt{parallel}
block.

\subsubsection{Examples}\label{set-background-examples}

\paragraph{Minimal --- switch to Venus blue at scene
start}\label{set-background-venus}

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{[}
  \FunctionTok{\{}\DataTypeTok{"action"}\FunctionTok{:} \StringTok{"set\_background"}\FunctionTok{,} \DataTypeTok{"color"}\FunctionTok{:} \StringTok{"\#1a3a6a"}\FunctionTok{\}}\OtherTok{,}
  \FunctionTok{\{}\DataTypeTok{"action"}\FunctionTok{:} \StringTok{"cast"}\FunctionTok{,} \DataTypeTok{"characters"}\FunctionTok{:} \FunctionTok{\{} \OtherTok{...} \FunctionTok{\}\}}\OtherTok{,}
  \FunctionTok{\{}\DataTypeTok{"action"}\FunctionTok{:} \StringTok{"fade\_in"}\FunctionTok{,} \DataTypeTok{"who"}\FunctionTok{:} \StringTok{"thalia"}\FunctionTok{\}}
\OtherTok{]}
\end{Highlighting}
\end{Shaded}

\paragraph{Scene-to-scene location
signaling}\label{set-background-location-signal}

The pattern below uses \texttt{set\_background} at every scene
boundary to make planet visible at a glance. Venus scenes receive
deep blue; Earth scenes receive black.

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{[}
  \FunctionTok{\{}\DataTypeTok{"\_comment"}\FunctionTok{:} \StringTok{"── SCENE 6: Venus, Avatar Control Conf. ──"}\FunctionTok{\}}\OtherTok{,}
  \FunctionTok{\{}\DataTypeTok{"action"}\FunctionTok{:} \StringTok{"set\_background"}\FunctionTok{,} \DataTypeTok{"color"}\FunctionTok{:} \StringTok{"\#1a3a6a"}\FunctionTok{\}}\OtherTok{,}
  \FunctionTok{\{}\DataTypeTok{"action"}\FunctionTok{:} \StringTok{"cast"}\FunctionTok{,} \DataTypeTok{"characters"}\FunctionTok{:} \FunctionTok{\{} \OtherTok{...} \FunctionTok{\}\}}\OtherTok{,}
  \OtherTok{...}

  \FunctionTok{\{}\DataTypeTok{"\_comment"}\FunctionTok{:} \StringTok{"── SCENE 7: Earth, PolymerCorp ──"}\FunctionTok{\}}\OtherTok{,}
  \FunctionTok{\{}\DataTypeTok{"action"}\FunctionTok{:} \StringTok{"set\_background"}\FunctionTok{,} \DataTypeTok{"color"}\FunctionTok{:} \StringTok{"\#000000"}\FunctionTok{\}}\OtherTok{,}
  \FunctionTok{\{}\DataTypeTok{"action"}\FunctionTok{:} \StringTok{"cast"}\FunctionTok{,} \DataTypeTok{"characters"}\FunctionTok{:} \FunctionTok{\{} \OtherTok{...} \FunctionTok{\}\}}\OtherTok{,}
  \OtherTok{...}
\OtherTok{]}
\end{Highlighting}
...
\paragraph{Relationship to \texttt{\_shot\_meta}
\texttt{bg\_color}}\label{set-background-vs-shot-meta}

The \texttt{bg\_color} key inside a \texttt{\_subscene\_marker} /
\texttt{\_shot\_meta} block (§5.8) also sets the camera background,
but only fires when the block triggers a camera move. The two
mechanisms are independent:

\begin{itemize}
\tightlist
\item
  Use \texttt{\_shot\_meta} \texttt{bg\_color} when the color change
  is part of a fly-in or descent sequence and you want it to coincide
  with the camera move.
\item
  Use \texttt{set\_background} when you want the color change at a
  plain cut with no camera motion, or when you need precise placement
  before the first \texttt{fade\_in} of a new scene.
\end{itemize}

If both appear in sequence, the last one to execute wins.

\subsubsection{Implementation
note}\label{set-background-implementation}

The handler lives in \texttt{pam\_player.py}'s top-level action
dispatch, immediately before the \texttt{remove\_scene\_object} block:

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{if} \BuiltInTok{act} \OperatorTok{==} \StringTok{"set\_background"}\NormalTok{:}
    \NormalTok{color }\OperatorTok{=} \NormalTok{step.get(}\StringTok{"color"}\NormalTok{)}
    \ControlFlowTok{if} \NormalTok{color:}
        \NormalTok{self.camera.background\_color }\OperatorTok{=} \NormalTok{color}
    \ControlFlowTok{continue}
\end{Highlighting}
\end{Shaded}

Because it assigns directly to \texttt{self.camera.background\_color}
rather than wrapping the change in a Manim \texttt{Animation} object,
the change is instantaneous and incurs no render overhead. It is safe
to call multiple times per scene (e.g.\ to simulate a flickering
environment), but it cannot be eased or interpolated without additional
custom code.

\subsubsection{Gotchas}\label{set-background-gotchas}

\begin{itemize}
\tightlist
\item
  \textbf{Not animatable.} There is no \texttt{rt} parameter. If you
  need a gradual background-color transition, you must implement a
  custom Manim animation.
\item
  \textbf{Persists until overwritten.} Like all Manim background
  settings, the color remains in effect for every subsequent frame
  until another \texttt{set\_background} (or a \texttt{\_shot\_meta}
  \texttt{bg\_color}) changes it. In a concatenated multi-scene render,
  always open each scene with an explicit \texttt{set\_background} so
  the background color is never inherited accidentally from the
  preceding scene.
\item
  \textbf{Cannot appear in a \texttt{parallel} block.} The action
  \texttt{continue}s immediately in the main loop without producing a
  Manim \texttt{Animation} object, so the two-pass parallel collector
  will find nothing to collect and the color change will not fire.
  Place it outside any \texttt{parallel} wrapper.
\end{itemize}

\section{11. Runtime additions in
v0.9.21}\label{runtime-additions-in-v0.9.21}

Three production-readiness features from the backburner (items 1--3),
all aimed at the multi-scene concatenated-render workflow used by
TNTD Act 2.

\subsection{\texorpdfstring{11.1 The cast+props
sidecar}{11.1 The cast+props sidecar}}\label{the-cast-props-sidecar}\index{sidecar}\index{PAM\_SIDECAR@\texttt{PAM\_SIDECAR}}\index{pam\_sidecar.json@\texttt{pam\_sidecar.json}}\index{cast@\texttt{cast}!sidecar}\index{faces@\texttt{faces}!sidecar}\index{props@\texttt{props}!sidecar}\index{scene\_props@\texttt{scene\_props}!sidecar}

A \emph{sidecar} is an optional shared file of preamble declarations
loaded alongside the scene file, so a multi-scene production can keep
one canonical character and prop roster instead of duplicating it in
every scene file.

\textbf{Resolution order.}

\begin{enumerate}
\item
  The \texttt{PAM\_SIDECAR} environment variable, if set. Because the
  author asked for this file explicitly, a missing file prints a
  warning (the render continues without it).
\item
  Otherwise \texttt{pam\_sidecar.json} in the same directory as
  \texttt{PAM\_SCRIPT}. If absent, the merge is a silent no-op.
\end{enumerate}

\begin{verbatim}
PAM_SCRIPT=scene_12.json \
PAM_SIDECAR=tntd_cast_props.json \
manim -pql pam_player.py PAMPlayer
\end{verbatim}

\textbf{File format.} A JSON array of action blocks, exactly like a
screenplay preamble, but restricted to the four declaration types:
\texttt{cast}, \texttt{faces}, \texttt{props}, and
\texttt{scene\_props}. Any other action type found in a sidecar is
ignored with a single warning line --- a sidecar declares the world, it
does not animate it. (Consequence: an existing showcase file such as
\texttt{tntd\_characters.json} works as a sidecar as-is; its
animation blocks are simply skipped.)

\textbf{Merge semantics.} Scene definitions take priority on
collision, at \emph{entry} granularity: if the scene's own
\texttt{cast} block defines character key \texttt{bevers}, the
sidecar's \texttt{bevers} entry is dropped in full --- keys are not
merged field-by-field. The same rule applies per face key and per prop
name (\texttt{props} and \texttt{scene\_props} share one prop
namespace). Surviving sidecar blocks are spliced in at the head of the
action list, after a leading \texttt{title} block if one is present,
preserving preamble-before-body ordering.

\textbf{Gotchas.}

\begin{itemize}
\item
  A sidecar containing a \texttt{cast} block switches the player into
  multi-character mode (\texttt{multi\ =\ True}), exactly as an inline
  \texttt{cast} block would.
\item
  Because collision handling is whole-entry replacement, a scene that
  wants to tweak one field of a shared character (say, its starting
  \texttt{offset}) must restate the full entry. Prefer overriding
  position at \texttt{fade\_in} time (\texttt{"x"} or
  \texttt{"offset"} on the \texttt{fade\_in} step) and leaving the
  sidecar entry untouched.
\item
  Sidecar \texttt{props} / \texttt{scene\_props} are built at time
  zero with the usual fade-in. A mid-scene \texttt{spawn\_prop} that
  reuses a sidecar prop name is \emph{not} treated as a collision; as
  with any duplicate name, the later registration overwrites the
  registry entry and can leave a ghost mob. Keep prop names unique.
\end{itemize}

\subsection{\texorpdfstring{11.2 \texttt{end\_scene} --- one-step
teardown}{11.2 end\_scene --- one-step teardown}}\label{end-scene-one-step-teardown}\index{end\_scene@\texttt{end\_scene}}\index{teardown!end\_scene@\texttt{end\_scene}}\index{PropRegistry@\texttt{PropRegistry}!end\_scene@\texttt{end\_scene}}\index{\_persistent\_bubbles@\texttt{\_persistent\_bubbles}!end\_scene@\texttt{end\_scene}}\index{BG\_COLOR@\texttt{BG\_COLOR}}

\begin{verbatim}
{"action": "end_scene"}
{"action": "end_scene", "rt": 0.8}
\end{verbatim}

One action closes out a scene inside a concatenated multi-scene file.
It performs, in order:

\begin{enumerate}
\item
  Removes every live prop in the \texttt{PropRegistry} ---
  preamble-declared and \texttt{spawn\_prop}'d alike --- \emph{except}
  dual-registered dog characters. A \texttt{pam\_dog} registry entry is
  a character (Path C), and characters are unaffected by
  \texttt{end\_scene}.
\item
  Clears every scene object (facades, backdrops, walls) from
  \texttt{\_scene\_objects}.
\item
  Dismisses all persistent speech bubbles, clearing both the player
  registry and the figure-side \texttt{\_persistent\_bubble} refs. This
  sidesteps the v0.9.10 \texttt{focus\_reset}/bubble-overlay class of
  stale-mob bugs at scene boundaries.
\item
  Resets the background to the player default (\texttt{BG\_COLOR},
  \texttt{\#0a0e1a}).
\end{enumerate}

\textbf{Keys.} \texttt{rt} --- removal run time in seconds. Default
\texttt{0.0}: instant removal, a hard cut into the next scene. With
\texttt{rt\ \textgreater{}\ 0}, props, scene objects, and bubbles all
fade concurrently in a single \texttt{play()} call.

\textbf{Gotchas.}

\begin{itemize}
\item
  Characters are left standing. \texttt{fade\_out} them explicitly (or
  keep them for the next scene of a walk-and-talk).
\item
  \texttt{end\_scene} resets the background to the \emph{default},
  not to the next scene's color. Per the §10.5 convention, still open
  every scene with an explicit \texttt{set\_background}.
\item
  Like \texttt{set\_background}, \texttt{end\_scene} lives in the main
  loop and cannot appear inside a \texttt{parallel} block.
\end{itemize}

\subsection{\texorpdfstring{11.3 Cast-level glove/shoe
defaults}{11.3 Cast-level glove/shoe defaults}}\label{cast-level-glove-shoe-defaults}\index{glove\_color@\texttt{glove\_color}}\index{glove\_stroke@\texttt{glove\_stroke}}\index{glove\_size@\texttt{glove\_size}}\index{shoe\_color@\texttt{shoe\_color}}\index{shoe\_stroke@\texttt{shoe\_stroke}}\index{shoe\_size@\texttt{shoe\_size}}\index{attach\_gloves@\texttt{attach\_gloves}!cast-level defaults}\index{attach\_shoes@\texttt{attach\_shoes}!cast-level defaults}\index{cast@\texttt{cast}!glove/shoe defaults}\index{fade\_in@\texttt{fade\_in}!glove/shoe defaults}

Six optional keys on a \texttt{cast} entry dress a character
automatically the moment it fades in:
...
When \texttt{glove\_color} is present, the player synthesizes an
\texttt{attach\_gloves} step and runs it immediately after that
character's \texttt{fade\_in} completes (instant --- no animation, so
the character appears already dressed); likewise
\texttt{shoe\_color} triggers \texttt{attach\_shoes}. The
\texttt{\_stroke} and \texttt{\_size} keys are optional refinements
and fall back to the attach handlers' own defaults
(\texttt{\#18120c}; \texttt{0.22} / \texttt{0.32}
\texttt{*\ fig.\_scale\_sy}).

\textbf{Override semantics.} Explicit \texttt{attach\_gloves} /
\texttt{attach\_shoes} actions in the scene body still work and
override the defaults: the figure-level attach methods are idempotent
and tear down any prior pair first. \texttt{detach\_gloves} /
\texttt{detach\_shoes} likewise remove the auto-applied pair.

\textbf{Gotchas.}

\begin{itemize}
\item
  Cast-level only: these keys are read from the \texttt{cast} entry,
  not from the \texttt{fade\_in} step. In single-character mode (no
  \texttt{cast} block) there is no place to declare them; use explicit
  attach actions.
\item
  Putting \texttt{glove\_color} on a \texttt{dog} or
  \texttt{governor} entry is harmless but noisy: the attach handler
  finds no wrist/ankle joints and prints a skip warning at every
  \texttt{fade\_in}.
\item
  These keys combine naturally with the sidecar (§11.1): declare the
  wardrobe once in \texttt{tntd\_cast\_props.json} and delete the
  per-scene \texttt{attach\_gloves} / \texttt{attach\_shoes}
  boilerplate that currently follows every \texttt{fade\_in}.
\end{itemize}

\subsection{\texorpdfstring{11.4 Rotation-aware \texttt{say}
anchoring}{11.4 Rotation-aware say anchoring}}\label{rotation-aware-say-anchoring}\index{say@\texttt{say}!rotation-aware anchor}\index{rotate@\texttt{rotate}!speech bubbles after}\index{bubble anchoring!after rotate@after \texttt{rotate}}

Backburner item 5, closed. \texttt{HumanGraph.say()} (inherited by
\texttt{AlienGraph}) and \texttt{DogGraph.say()} now anchor the speech
bubble to the \emph{live} head-dot position
(\texttt{self.dots{[}"head"{]}.get\_center()}) instead of computing it
from pose space (\texttt{self.pose} + \texttt{self.offset}).

Why it matters: \texttt{rotate} is a visual-only \texttt{VGroup}
transform --- it never updates \texttt{self.pose} or
\texttt{self.offset} (§2.5). Under the old pose-space anchor, a
character rotated to lie flat would speak from where its head
\emph{used} to be. Reading the rendered dot directly places the bubble
correctly after any rotation, including the lying-flat case. For
unrotated characters the dot position equals the pose-space position,
so existing scenes are unaffected.

\textbf{Remaining edges.}

\begin{itemize}
\item
  \texttt{GovernorGraph.say()} still anchors from
  \texttt{self.\_y\ +\ self.\_radius}; governors have no head dot and
  do not rotate (their spin is internal), so this is by design. Use
  its \texttt{y\_offset} parameter for per-call nudges.
\item
  The bubble still pops \emph{above} the anchor. For a character lying
  flat this reads fine; there is no tail-direction reflow. Rotation-aware
  \texttt{morph\_to} (backburner item 4) remains open and is unrelated
  to this fix.
\end{itemize}

\subsection{\texorpdfstring{11.5 Console logging:
\texttt{PAM\_LOG} and
\texttt{PAM\_WARNINGS\_ONLY}}{11.5 Console logging: PAM\_LOG and PAM\_WARNINGS\_ONLY}}\label{console-logging-pam-log}\index{PAM\_LOG@\texttt{PAM\_LOG}}\index{PAM\_WARNINGS\_ONLY@\texttt{PAM\_WARNINGS\_ONLY}}\index{logging!console tee}\index{warnings-only log}\index{pam-render@\texttt{pam-render}!--log / --warnings-only@\texttt{-{}-log} / \texttt{-{}-warnings-only}}

Two environment variables tee console output to a file for
post-render inspection --- indispensable for long concatenated
renders where a skipped-action warning scrolls past hours before
anyone reads it.

\begin{verbatim}
PAM_SCRIPT=scene.json PAM_LOG=render.log \
manim -pql pam_player.py PAMPlayer

PAM_SCRIPT=scene.json PAM_WARNINGS_ONLY=warnings.log \
manim -pql pam_player.py PAMPlayer
\end{verbatim}

\begin{itemize}
\item
  \texttt{PAM\_LOG} --- everything that reaches the console (stdout
  \emph{and} stderr, including Manim's own output) is also written to
  the file.
\item
  \texttt{PAM\_WARNINGS\_ONLY} --- only PAM-originated lines are
  written: those beginning with a prefix in
  \texttt{\_PAM\_LOG\_PREFIXES} (\texttt{PAMPlayer},
  \texttt{PAM\ props}, \texttt{PAM:}). Manim's render chatter and
  progress bars are excluded, leaving a clean warning digest.
\end{itemize}

Both channels may be active simultaneously (different files). Pointing
both at the \emph{same} path is refused: \texttt{PAM\_WARNINGS\_ONLY}
wins and \texttt{PAM\_LOG} is ignored with a notice. The tee installs
at module import time --- before Manim loads --- so nothing escapes
it; console output itself is unchanged. Log files are opened in write
mode (one render, one log). An unopenable path skips that channel with
a notice and the render proceeds.

Because Manim's CLI \emph{imports} \texttt{pam\_player.py} rather than
executing it as \texttt{\_\_main\_\_}, these are environment variables
rather than command-line flags. The \texttt{pam-render} wrapper can
expose them as \texttt{-{}-log\ FILE} / \texttt{-{}-warnings-only\ FILE}
flags by setting the corresponding variable in the child environment.

Future (Option C, still on the backburner): a structured warning
registry accumulating \texttt{(array\_index,\ action\_type,\ message)}
tuples, dumped as JSON at render completion.

\subsection{\texorpdfstring{11.6 \texttt{pam\_merge\_audio.py} ---
post-render soundtrack
merge}{11.6 pam\_merge\_audio.py --- post-render soundtrack merge}}\label{pam-merge-audio}\index{pam\_merge\_audio.py@\texttt{pam\_merge\_audio.py}}\index{audio!post-render merge}\index{soundtrack merge}\index{pam\_merge\_audio.py@\texttt{pam\_merge\_audio.py}!--loop@\texttt{-{}-loop}}\index{pam\_merge\_audio.py@\texttt{pam\_merge\_audio.py}!--trim@\texttt{-{}-trim}}\index{pam\_merge\_audio.py@\texttt{pam\_merge\_audio.py}!--offset@\texttt{-{}-offset}}\index{pam\_merge\_audio.py@\texttt{pam\_merge\_audio.py}!--volume@\texttt{-{}-volume}}\index{pam\_merge\_audio.py@\texttt{pam\_merge\_audio.py}!--mix@\texttt{-{}-mix}}\index{FFmpeg!pam\_merge\_audio.py@\texttt{pam\_merge\_audio.py}}

Backburner item 8. A standalone CLI (thin FFmpeg wrapper) that merges
a soundtrack into an already-rendered video, entirely outside Manim.
The video stream is stream-copied --- no re-encode, no quality loss,
and fast even on a feature-length file.

\begin{verbatim}
python3 pam_merge_audio.py scene.mp4 soundtrack.wav
python3 pam_merge_audio.py scene.mp4 soundtrack.wav -o scene_final.mp4
python3 pam_merge_audio.py scene.mp4 theme.m4a --loop --volume 0.6
python3 pam_merge_audio.py scene.mp4 stinger.ogg --offset 3.5
python3 pam_merge_audio.py scene.mp4 score.flac --mix
\end{verbatim}

\textbf{Length semantics.} The output is always exactly as long as
the input video. \texttt{-{}-trim} (default): audio longer than the
video is cut at the video's end; audio shorter than the video simply
ends early. \texttt{-{}-loop}: the audio is looped end-to-end to cover
the full video length, then cut.

\textbf{Options.}

\begin{itemize}
\item
  \texttt{-{}-offset\ SECONDS} --- positive delays the audio start
  (silence first); negative skips into the audio file (start
  mid-track).
\item
  \texttt{-{}-volume\ FLOAT} --- soundtrack volume multiplier
  (\texttt{1.0} unchanged, \texttt{0.5} half). Applied before mixing.
\item
  \texttt{-{}-mix} --- mix with the video's existing audio track
  instead of replacing it. Intended for renders that already carry
  baked in-scene sound cues (backburner item 7). On a silent video,
  \texttt{-{}-mix} falls back to replace with a notice.
\item
  \texttt{-o\ /\ -{}-output\ PATH} --- default
  \texttt{\textless{}video\ stem\textgreater{}\_final.mp4}; refuses to
  overwrite an input file.
\end{itemize}

Supported soundtrack formats: \texttt{.wav}, \texttt{.ogg},
\texttt{.m4a}, \texttt{.mp3}, \texttt{.flac} (all handled natively by
FFmpeg --- no temp-file conversion step is needed here, unlike the
planned in-scene cue system). Output audio is encoded AAC at
192\,kb/s. Requires \texttt{ffmpeg} and \texttt{ffprobe} on
\texttt{PATH} (\texttt{brew\ install\ ffmpeg} on macOS).

\subsection{\texorpdfstring{11.7 In-scene audio
cues}{11.7 In-scene audio cues}}\label{in-scene-audio-cues}\index{audio@\texttt{audio} block}\index{sound@\texttt{sound} key}\index{audio!in-scene cues}\index{add\_sound@\texttt{add\_sound}}\index{m4a conversion@\texttt{.m4a} conversion}\index{sound\_cue@\texttt{sound\_cue}!with real audio}\index{parallel@\texttt{parallel}!audio cues}

Backburner item 7. Real audio, baked into the rendered file via
Manim's \texttt{Scene.add\_sound()}. Two layers:

\textbf{1. Action-type defaults.} An optional
\texttt{\{"action":\ "audio",\ "defaults":\ \{…\}\}} block --- place
it in the preamble --- declares a default sound per action type,
keyed by the \emph{exact action name}:
...
dispatch time, i.e.\ at the moment the action starts (a \texttt{say}
default sounds at bubble fade-in). Multiple \texttt{audio} blocks
accumulate; later blocks override earlier defaults per key.

\textbf{2. Per-step \texttt{sound} key.} Any action step may carry
one:

\begin{verbatim}
{"action": "jump_up", "who": "bevers", "sound": "sfx/boing.wav"}
{"action": "say", "who": "bevers", "text": "…", "sound": null}
{"action": "wait", "rt": 2.0, "sound": "sfx/ambience.wav"}
\end{verbatim}

A per-step \texttt{sound} overrides the action-type default; an
explicit \texttt{null} (or \texttt{false} or \texttt{""}) suppresses
the default for that one step. \emph{Any} action can carry a
\texttt{sound}, including \texttt{wait} (ambience under a pause) and
\texttt{sound\_cue} (a real sound alongside the on-screen caption).

\textbf{Paths and formats.} Absolute paths are used as-is; relative
paths resolve against the scene file's directory first, then the
current working directory. \texttt{.wav}, \texttt{.ogg},
\texttt{.mp3}, and \texttt{.flac} are passed to Manim natively.
\texttt{.m4a} files are transparently converted to \texttt{.wav} via
\texttt{ffmpeg} into a temp directory at first use --- cached per
file, cleaned up at exit --- so the scene author just writes the
\texttt{.m4a} path. A missing file, missing \texttt{ffmpeg}, failed
conversion, or unsupported extension warns once per path and the
render continues silent.

\textbf{Timing.}

\begin{itemize}
\item
  Cues fire when the step is dispatched. There is no per-cue delay or
  gain control at this layer --- that ladder
  (\texttt{sfx\_gain}, \texttt{sfx\_delay}, \texttt{sfx\_trigger}) is
  the per-character SFX system, backburner item 18, targeted at
  PAM 1.0.1.
\item
  Inside a \texttt{parallel} block, every sub-step's cue fires at the
  block's start time, matching the simultaneity semantics of the
  block itself.
\item
  Manim only mixes audio into the output when the render actually
  produces a file; audio is included in normal \texttt{-ql} through
  \texttt{-qh} renders.
\end{itemize}

\textbf{Interplay with \texttt{pam\_merge\_audio.py}} (§11.6): a
render carrying baked in-scene cues still takes a soundtrack cleanly
--- use \texttt{-{}-mix} so the merge blends with the cues instead of
replacing them.

\subsection{\texorpdfstring{11.8 v0.9.21 fixes and developer
tooling}{11.8 v0.9.21 fixes and developer tooling}}\label{v0921-fixes-and-tooling}\index{parallel@\texttt{parallel}!walk\_to on a dog@\texttt{walk\_to} on a dog}\index{\_tag\_dog\_group@\texttt{\_tag\_dog\_group}}\index{pam\_rag\_index.py@\texttt{pam\_rag\_index.py}}\index{pam\_rag\_query.py@\texttt{pam\_rag\_query.py}}\index{RAG (manual search)}

\textbf{Fix: \texttt{walk\_to} / \texttt{run\_to} on a dog inside
\texttt{parallel}} (item 14). Path C made cast-loaded dogs reachable
through the humanoid locomotion path, which called
\texttt{\_walk\_plan()} --- absent on \texttt{DogGraph} --- and
crashed. A dog asked to walk now trots, with a console notice; the
\texttt{stride} key is honored.

\textbf{Internal: \texttt{\_tag\_dog\_group()}} (item 15). The
prop-side wrap-and-tag block for dual-registered dogs, previously
duplicated between \texttt{fade\_in} and \texttt{spawn\_prop}, is now
one module-level helper that also owns the fresh-VGroup binding
gotcha. No behavior change.

\textbf{Developer tooling} (item 9): \texttt{pam\_rag\_index.py}
chunks this manual into \texttt{pam\_rag\_chunks.json} (local BM25, no
dependencies); \texttt{pam\_rag\_query.py} searches it from the
command line, with an optional \texttt{-{}-ask} Claude-API answer
step. For sessions \emph{with} Claude, keep attaching the manual
directly.

\section{12. Runtime additions in
v0.9.22}\label{runtime-additions-in-v0.9.22}

\subsection{\texorpdfstring{12.1 Rotation-aware
\texttt{morph\_to}}{12.1 Rotation-aware morph\_to}}\label{rotation-aware-morph-to}\index{morph\_to@\texttt{morph\_to}!rotation-aware}\index{set\_pose@\texttt{set\_pose}!rotation-aware}\index{note\_rotation@\texttt{note\_rotation}}\index{\_pam\_rotation@\texttt{\_pam\_rotation}}\index{rotate@\texttt{rotate}!morph after}\index{\_rot\_xy@\texttt{\_rot\_xy}}

Backburner item 4, the last 1.0.0 core item. Previously,
\texttt{rotate} was a visual-only VGroup transform: it accumulated
\texttt{\_pam\_rotation} for face placement but never updated
\texttt{fig.pose} / \texttt{fig.offset}, so any subsequent
\texttt{morph} animated toward \emph{unrotated} pose-space
coordinates --- a lying-flat character would visually snap upright
and back to its pre-rotation spot.

\textbf{The invariant.} As of v0.9.22, \texttt{HumanGraph} (and
\texttt{AlienGraph} by inheritance) and \texttt{DogGraph} place every
joint by

\[ \mathrm{world} = R(\theta)\, p_{\mathrm{scaled}} + \mathrm{offset} \]

where $\theta$ is \texttt{fig.\_pam\_rotation} (initialized to $0$)
and \texttt{fig.offset} is the figure's effective translation. A
\texttt{rotate} by $\delta$ about world pivot $v$ updates both terms
via \texttt{note\_rotation()}:

\[ \theta \leftarrow \theta + \delta, \qquad
\mathrm{offset} \leftarrow R(\delta)(\mathrm{offset} - v) + v . \]

Because every composition of planar rotations and translations is
again of this form, \emph{any} sequence of \texttt{rotate} actions
about \emph{different} pivots remains exactly representable by the
single $(\theta, \mathrm{offset})$ pair --- no pivot is stored, and
there is no composite-pivot bookkeeping to get wrong. With
$\theta = 0$ the invariant reduces to the pre-v0.9.22 placement, so
unrotated scenes are unaffected.

\textbf{What now works.}

\begin{itemize}
\item
  \texttt{morph} / \texttt{morph\_to} on a rotated figure renders the
  new pose in the rotated, relocated frame (a character lying flat
  can change pose while staying flat and in place).
\item
  \texttt{set\_pose} (and everything built on it) likewise.
\item
  An exact inverse rotation about the same pivot returns the figure
  bit-exactly to its pre-rotation state ($\theta = 0$, original
  offset), after which all legacy behavior applies.
\item
  \texttt{morph}'s \texttt{dx}/\texttt{dy} remain clean world-space
  nudges under rotation.
\end{itemize}

\textbf{Still undefined under rotation} (unchanged scope):
locomotion (\texttt{walk\_to}, \texttt{run\_to}, \texttt{trot\_to})
and \texttt{turn} assume an upright figure. Rotate back to
$\theta = 0$ (or morph) before walking a figure.

\subsection{\texorpdfstring{12.2 Named rotation
pivots}{12.2 Named rotation pivots}}\label{named-rotation-pivots}\index{rotate@\texttt{rotate}!named pivot}\index{pivot!named (character/prop)}

With the companion \texttt{act\_rotate} patch, a \texttt{pivot}
string that is not a positional keyword (\texttt{bottom},
\texttt{center}, \texttt{top}, \texttt{left}, \texttt{right})
resolves as a character or prop name; the rotation pivots about that
target's live center:

\begin{verbatim}
{"action": "rotate", "who": "chekov",
 "angle_deg": 180, "pivot": "bevers", "rt": 1.6}
\end{verbatim}

Cast names are checked first, then the prop registry; an unknown name
falls back to \texttt{bottom} with a warning. The same patch also
switches figure-pivot derivation (\texttt{bottom}, \texttt{top},
\ldots) to live dot positions, so pivots are correct on
already-rotated figures.

\subsection{\texorpdfstring{12.3 Cast
merge-on-redefine}{12.3 Cast merge-on-redefine}}\label{cast-merge-on-redefine}\index{cast@\texttt{cast}!merge-on-redefine}\index{cast@\texttt{cast}!mid-scene redefinition}\index{scale@\texttt{scale}!lost on redefinition (fixed)}

Backburner item 19. A mid-scene \texttt{cast} block that redefines an
existing character previously \emph{replaced} its entry wholesale:
any key not restated (\texttt{scale}, \texttt{style},
\texttt{tic\_profile}, glove/shoe defaults, \ldots) silently reverted
to defaults at the next \texttt{fade\_in} --- the "character comes
back at the wrong size" bug.

Now the redefinition \emph{merges}: keys the new block states win;
unstated keys are inherited from the prior entry, with a console
notice listing what was inherited. \texttt{style} merges one level
deep (new color keys override, unmentioned ones persist) and is
copied, never aliased. Three carve-outs always take the new block's
value: \texttt{figure\_type} (changing type is intentional) and
\texttt{pose} / \texttt{offset} (scene-positional). To force a key
back to its default, state it explicitly:
...
\subsection{\texorpdfstring{12.4 \texttt{punch} --- character-on-character
hit}{12.4 punch --- character-on-character hit}}\label{punch-action}\index{punch@\texttt{punch}}\index{fight choreography!punch@\texttt{punch}}\index{recoil (punch target)}\index{reach\_character@\texttt{reach\_character}!vs.\ punch@vs.\ \texttt{punch}}

One character punches another: a sharp jab from the puncher plus a
synchronized recoil --- and optional knockback stagger --- on the
target, in a single action. The puncher's half reuses the
\texttt{reach\_character} jab geometry; the target's reaction is what
makes the beat read as a \emph{hit}. (\texttt{punch\_button} remains
the prop-jab action; pointing it at a character has never worked ---
the target lookup misses the prop registry and the jab lands at a
fallback point.)
...
\textbf{Keys.} \texttt{target} (cast member being hit);
\texttt{arm} (\texttt{"r"} default); \texttt{zone} ---
\texttt{"head"} (default), \texttt{"torso"}, \texttt{"shoulder"},
\texttt{"hip"}; \texttt{rt} (jab speed, default 0.15);
\texttt{hold} (contact dwell, 0.06); \texttt{power} (stagger
distance in world units, default 0.35; recoil lean scales with it);
\texttt{settle} --- \texttt{true} (default) recovers the target's
pose while keeping the knockback displacement, \texttt{false} leaves
the target reeling.

\textbf{Knockdown is a composition}, not a key --- follow with a
rotate, and (§12.1) the downed character can still morph afterward:
...
\textbf{Notes.} Impact point and puncher shoulder read from live
dots, so the punch lands correctly after prior walks or rotations of
the target (a rotated \emph{puncher} is undefined --- rotate back
first). The recoil shifts only joints the target's build has, so a
dog target gets a modest shove rather than a crash. Not
parallel-safe. The recoil constants are first-render values ---
tune \texttt{power} to taste.

\subsection{\texorpdfstring{12.5 \texttt{dog\_sitting} and sloped-spine
dog poses}{12.5 dog\_sitting and sloped-spine dog poses}}\label{dog-sitting}\index{dog\_sitting@\texttt{dog\_sitting}}\index{poses!dog\_sitting@\texttt{dog\_sitting}}\index{dog\_side\_pose@\texttt{dog\_side\_pose}!sloped spine}\index{DOG\_SITTING@\texttt{DOG\_SITTING}}

\begin{verbatim}
{"action": "morph", "who": "chekov",
 "pose": "dog_sitting", "rt": 0.8}
\end{verbatim}

Classic sit, side view: haunches on the ground, hind legs folded
forward under the body, front legs vertical and planted, spine
sloping up to a chest at standing height, head carried high, tail
resting behind. Ground contact matches standing (paws and haunches
at $y = -0.65$ in dog-local coordinates), and every leg and spine
segment holds its standing length to within a few percent, so the
stand$\leftrightarrow$sit morph reads as joints folding rather than
limbs stretching. \texttt{DOG\_SITTING\_LEFT} exists for left-facing
spawns.

\textbf{Builder extension.} \texttt{dog\_side\_pose()} gained three
optional parameters --- \texttt{spine\_front\_y},
\texttt{spine\_mid\_y}, \texttt{spine\_rear\_y}, each defaulting to
\texttt{spine\_y} --- because a sit is definitionally a sloped spine
and the builder previously forced all three spine joints to one
height. Fully backward-compatible; existing poses are unchanged.
Future sloped-spine poses (play-bow, stretch) build on the same
parameters.

\textbf{Facing caveat} (pre-existing convention, not new): registry
dog poses are right-facing --- a left-facing dog morphing to any
registry pose gets the right-facing coordinates. Facing-aware
registry resolution is a candidate 1.0.1 item.

\section{13. Runtime additions in
v0.9.23}\label{runtime-additions-in-v0.9.23}

\subsection{\texorpdfstring{13.1 The 2.5D depth system: overview}{13.1 The 2.5D depth system: overview}}\label{the-2-5d-depth-system-overview}\index{2.5D depth system}\index{depth@\texttt{depth}}\index{rescale@\texttt{rescale}}\index{depth\_config@\texttt{depth\_config}}

Backburner items 10--11. An optional depth model for cast and props
entries, giving PAM a lightweight sense of "near" and "far" without a
true 3-D camera. Two JSON keys, usable on any \texttt{cast} entry or
any \texttt{props}/\texttt{scene\_props}/\texttt{spawn\_prop} entry:

\begin{itemize}
\item
  \texttt{"depth"} (float, default \texttt{0.0}) --- \texttt{0} is the
  on-stage plane; negative values are farther from the camera.
  Positive (foreground) depth is not supported in 1.0.1: it is warned
  once per name and clamped to \texttt{0}.
\item
  \texttt{"rescale"} (bool, default \texttt{false}) --- opt-in
  perspective shrink. Depth \emph{without} \texttt{rescale} affects
  only z-layering (\S13.3); the entry's on-screen size is untouched ---
  useful for a backdrop or prop that was already drawn at the correct
  scale for its position. \texttt{"rescale": true} at depth \texttt{0}
  is a no-op (warned once): the scale law gives \texttt{1.0} there
  regardless.
\end{itemize}

Both keys participate in the ordinary mid-scene \texttt{cast}
merge-on-redefine (backburner item 19,
\S\ref{cast-merge-on-redefine}): a redefinition
that doesn't restate \texttt{"depth"}/\texttt{"rescale"} inherits the
character's prior value, exactly like \texttt{"scale"} or
\texttt{"style"}.

\begin{verbatim}
{"action": "cast", "characters": {
  "sidel": {
    "pose": "standing_front", "offset": [-4, 0, 0],
    "depth": -5, "rescale": true
  }
}}
\end{verbatim}

\subsection{\texorpdfstring{13.2 The scale law and \texttt{depth\_config}}{13.2 The scale law and depth\_config}}\label{the-scale-law-and-depth-config}\index{depth\_k@\texttt{depth\_k}}\index{depth\_scale@\texttt{depth\_scale}}

Perspective strength is a single scene-level constant $k$ (default
\texttt{0.08}), set via an optional preamble action --- mirroring the
existing \texttt{\{"action": "audio", "defaults": \{...\}\}} pattern,
order-dependent the same way: put it before any \texttt{cast}/
\texttt{props} block that needs a non-default $k$.

\begin{verbatim}
{"action": "depth_config", "depth_k": 0.08}
\end{verbatim}

A rescaled entry's scale factor is

\[ s_{\mathrm{depth}} = \frac{1}{1 - k \cdot d} \]

where $d$ is the entry's depth. Reference values at $k = 0.08$:

\begin{center}
\begin{tabular}{r r}
depth $d$ & scale $s_{\mathrm{depth}}$ \\
\hline
$0$    & $1.000$ \\
$-3$   & $0.806$ \\
$-5$   & $0.714$ \\
...
Stored on the figure as \texttt{fig.depth}, \texttt{fig.depth\_rescale},
\texttt{fig.depth\_scale} (and, for props, the \texttt{pam\_depth} /
\texttt{pam\_depth\_rescale} / \texttt{pam\_depth\_scale} flat
attributes, alongside the existing \texttt{pam\_x}/\texttt{pam\_y}/
\texttt{pam\_surface\_y} convention documented in the Prop Properties
chapter). \texttt{fig.\_depth\_k} is a snapshot of the scene's $k$
taken at \texttt{fade\_in}/\texttt{spawn\_prop} time, used by
depth-animated locomotion (\S13.5) to re-derive the law without a
back-reference to the scene; like every other PAM preamble constant, a
\texttt{depth\_config} change mid-scene does not retroactively update
already-spawned figures.

\subsection{\texorpdfstring{13.3 Z-index bands}{13.3 Z-index bands}}\label{z-index-bands}\index{z\_index@\texttt{z\_index}!depth bands}\index{apply\_depth\_z@\texttt{apply\_depth\_z}}\index{DEPTH\_Z\_BACKGROUND@\texttt{DEPTH\_Z\_BACKGROUND}}\index{DEPTH\_Z\_BUBBLE@\texttt{DEPTH\_Z\_BUBBLE}}

World-layer figures and props are banded to \texttt{z\_index = depth}
directly, so more-negative depth sorts behind less-negative depth.
Bubbles/captions sit far in front (\texttt{DEPTH\_Z\_BUBBLE = 1000});
a conceptual background band (\texttt{DEPTH\_Z\_BACKGROUND = -1000})
is reserved but not currently wired to anything --- the existing
\texttt{scene\_objects} \texttt{"backdrop"} type still uses its
pre-existing \texttt{bring\_to\_back()} insertion-order mechanism,
which is orthogonal to \texttt{z\_index} and untouched by this work.
Intra-figure attachments (\texttt{head\_face}, \texttt{torso\_icon},
\texttt{harness}, \texttt{gloves}, \texttt{shoes}, the figure-attached
\texttt{\_name\_tag}) get a small fractional bump
(\texttt{DEPTH\_ATTACH\_Z\_STEP = 0.001} per attachment) above their
figure's band, so intra-figure layering (e.g.\ a face image above its
transparent head dot) survives depth-based sorting.

The single source of truth is \texttt{fig.apply\_depth\_z()}
(\texttt{HumanGraph}, inherited by \texttt{AlienGraph};
\texttt{DogGraph}; \texttt{GovernorGraph}), called at \texttt{fade\_in}/
\texttt{spawn\_prop}, whenever a mid-scene \texttt{cast} redefinition
changes depth and the character is re-spawned, and --- continuously
--- during depth-animated locomotion (\S13.5). An explicit sticky
\texttt{"z\_index"} (v0.9.19) always takes final precedence over a
depth-derived one, for both characters and props; \texttt{
\_reassert\_z\_indices()} (already called after every
\texttt{bring\_to\_front}/\texttt{bring\_to\_back} in the pre-existing
code) falls back to \texttt{apply\_depth\_z()} for any character
without a sticky z\_index, so a \texttt{bring\_to\_front} elsewhere in
the scene cannot silently pull a background-depth figure in front of a
foreground one.

Verified against the pinned Manim Community version for this session
(0.20.1): \texttt{manim.utils.family.extract\_mobject\_family\_members}
always does \texttt{sorted(family, key=lambda m: m.z\_index)} fresh, on
every render call --- Python's \texttt{sorted()} is stable, so
mobjects sharing a depth keep scene-insertion order, matching
pre-depth behavior exactly, and z\_index changes take effect
immediately on the next rendered frame with no caching to invalidate.

\textbf{Note for anyone debugging z\_index by hand:}
\texttt{fig.group} is a \emph{property} that constructs a fresh
\texttt{VGroup} wrapper on every access (true of \texttt{HumanGraph},
\texttt{AlienGraph}, and \texttt{DogGraph} alike, not just the
already-documented \texttt{dog.group} case). Calling
\texttt{.set\_z\_index()} on it still works correctly --- Manim's
\texttt{set\_z\_index(value, family=True)} recurses into submobjects by
default, so the value reaches the real, persistent dots/lines even
though the wrapper itself is discarded --- but \emph{reading}
\texttt{fig.group.z\_index} back afterward does \emph{not} reflect
that: it reads a brand-new wrapper's own default (\texttt{0}). Inspect
a real leaf mobject instead, e.g.\ \texttt{fig.dots["head"].z\_index}.

\subsection{\texorpdfstring{13.4 Placement-model integration: rescale and ground contact}{13.4 Placement-model integration}}\label{placement-model-integration-depth}\index{\_apply\_scale@\texttt{\_apply\_scale}!depth-scale}\index{ground contact}\index{depth\_scale@\texttt{depth\_scale}!ground contact}

PAM's pose-space origin is \emph{not} at the ground line ---
\texttt{STANDING\_FRONT["lankle"]} is $(-0.52, -2.6, 0)$, not on the
$x$-axis. \texttt{HumanGraph.\_apply\_scale} (inherited by
\texttt{AlienGraph}) composes body-scale and depth-scale as two
separate anchored calls to the existing \texttt{scale\_pose()} helper:
body-scale at \texttt{fig.\_scale\_anchor} (whatever joint the author
chose, default \texttt{"lankle"}), then depth-scale at
\texttt{fig.\_depth\_ground\_joint} (\texttt{"lankle"} for humans/
aliens, \texttt{"fl\_paw"} for \texttt{DogGraph}) \emph{regardless of
what anchor body-scale used} --- depth perspective is always
ground-anchored even if body-scale deliberately used a stylistic
non-ground anchor. Because \texttt{scale\_pose}'s anchor mechanism
leaves its anchor joint's position exactly unchanged by construction
($a + s(v-a)$, and $v = a$ at the anchor itself), composing the two
scales this way needs \emph{no} separate offset compensation on
\texttt{fig.offset} --- simpler than an alternative formula based on
adjusting the offset directly. \texttt{DogGraph} gained its first-ever
\texttt{\_apply\_scale} in this pass (depth-scale only; it has no
body-scale feature); \texttt{GovernorGraph} has no pose/ground system
at all (it floats), so its rescale (\texttt{apply\_depth\_rescale()})
is a plain uniform \texttt{.scale()} about its own centre.

This is the single choke point D1 calls for: \texttt{set\_pose},
\texttt{morph\_to}/\texttt{\_pose\_anims}, \texttt{turn},
\texttt{wave}, \texttt{nod}, and every other pose-based animation
already routes through \texttt{\_apply\_scale}, so they all pick up
depth-scale automatically. \texttt{say()} needed no change: it already
anchors from the \emph{live} head-dot position
(\texttt{fig.dots["head"].get\_center()}), correct automatically once
dot placement is correct. Rotation (\texttt{note\_rotation}, item 4)
composes for free too --- rotation and depth-scale are independent
stages (\texttt{\_apply\_scale} then \texttt{\_to\_world}), so a
depth-scaled figure rotates and morphs with no drift, teleport, or
resize, verified both numerically and via a real rotate/morph test
scene.

\textbf{Interaction actions.} \texttt{\_apply\_scale} multiplying in
depth-scale meant every place that converts a \emph{world-space} delta
into a \emph{pose-space} delta for \texttt{morph\_to} (so the
subsequent \texttt{\_apply\_scale} re-multiplying it lands back at the
intended world delta) needed the same factor divided back out. About
40 call sites across \texttt{actions.py} and
\texttt{actions\_interactions.py} (\texttt{point\_at},
\texttt{hand\_to}, \texttt{grab\_arm}/\texttt{release\_arm} rest-pose
reconstruction, and similar) migrated from the pre-existing
\texttt{1.0 / fig.\_scale\_sx if fig.is\_scaled else 1.0} pattern to
two new methods, \texttt{fig.\_inv\_scale\_x()} /
\texttt{fig.\_inv\_scale\_y()}, which fold in \texttt{depth\_scale}
alongside body-scale (also added to \texttt{DogGraph} for defensive
symmetry).

Props rescale similarly: \texttt{\_apply\_prop\_rescale(prop,
depth\_scale)} (\texttt{pam\_player.py}) does
\texttt{prop.scale(depth\_scale, about\_point=[pam\_x, pam\_y, 0])}
--- scaling about the same point \texttt{PropRegistry.world\_pos()}
already trusts means that point (typically a prop's floor-contact
coordinate) is provably invariant under rescale, with no changes
needed to \texttt{world\_pos()} itself.

\subsection{\texorpdfstring{13.5 Depth-animated locomotion}{13.5 Depth-animated locomotion}}\label{depth-animated-locomotion}\index{walk\_to@\texttt{walk\_to}!depth}\index{run\_to@\texttt{run\_to}!depth}\index{trot\_to@\texttt{trot\_to}!depth}\index{\_lerp\_depth\_state@\texttt{\_lerp\_depth\_state}}

\texttt{walk\_to}, \texttt{run\_to} (\texttt{HumanGraph}/
\texttt{AlienGraph}) and \texttt{trot\_to} (\texttt{DogGraph}) accept
an optional \texttt{"depth"} key:

\begin{verbatim}
{"action": "walk_to", "who": "sidel", "x": 4.0, "depth": 0}
\end{verbatim}

Depth is interpolated \emph{linearly} across the move's keyframes
(PAM's gait animation is already a discrete per-keyframe loop --- one
\texttt{morph\_to}/\texttt{scene.play()} call per gait pose, not one
continuous animation --- so "per frame" in the design sense naturally
became "per keyframe" here); \texttt{depth\_scale}, if
\texttt{depth\_rescale} is active, is \emph{re-derived from the
interpolated depth} via the scale law each keyframe, never linearly
interpolated itself (the reciprocal law's curvature is what reads as
perspective). After the move, \texttt{depth}/\texttt{depth\_scale}
land exactly on the target --- not just alpha-close --- and the
\texttt{cast[name]} dict is kept in sync so a later mid-scene
\texttt{cast} redefinition that doesn't restate \texttt{"depth"}
inherits the character's \emph{current} (post-walk) depth, not the
value it was originally spawned with.

z\_index is re-banded (\texttt{apply\_depth\_z()}) at \emph{every}
keyframe of a depth-animated move, not just once at the end --- this
is what lets a moving character's occlusion against a stationary one
flip at the correct moment mid-walk (verified: constructing the
figures directly and reading each keyframe's real z\_index shows the
occlusion flip landing exactly on the keyframe where interpolated
depth crosses the stationary figure's depth). This needed no polling
registry or per-render-frame updater --- see \S13.3's stable-sort
verification.

\textbf{Known gap:} a \texttt{walk\_to}/\texttt{run\_to}/
\texttt{trot\_to} call with the same \texttt{x} as the figure's
current position (a would-be depth-only move, no horizontal travel) is
a no-op, same as it always was pre-depth --- so a depth change without
any x displacement is silently skipped. Not exercised by any 1.0.1
scene design; a candidate 1.0.2 item if a real scene ever needs it.

\subsection{\texorpdfstring{13.6 Props and dual registration}{13.6 Props and dual registration}}\label{props-and-dual-registration}\index{dual registration!depth}\index{PropRegistry@\texttt{PropRegistry}!world\_pos and depth}

\texttt{"depth"}/\texttt{"rescale"} work identically on
\texttt{props}/\texttt{scene\_props} items and on all three
\texttt{spawn\_prop} branches (the standard prop-builder path, the
\texttt{GovernorGraph}/dodecahedron branch, and the \texttt{DogGraph}
Path-C dual-registration branch).

For dual-registered entities (a \texttt{cast}-declared dog later
materialized via \texttt{spawn\_prop} --- the only mechanism that
actually produces dual registration in PAM today; a generic
\texttt{props} item cannot declare \texttt{"type": "dog"}, since
\texttt{"dog"} is not in \texttt{PROP\_TYPES}), the \texttt{cast}
entry's depth wins. A mismatch between the two is warned once:

\begin{quote}
\texttt{PAMPlayer: depth --- 'chekov' is dual-registered with
mismatched depth (cast=-3, spawn\_prop=-1); using the cast value.}
\end{quote}

The precedence check is order-dependent, like every other PAM preamble
mechanism: the \texttt{cast} declaration must appear before the
conflicting \texttt{spawn\_prop}/\texttt{props} entry in the actions
array to be seen.

\subsection{\texorpdfstring{13.7 Warnings reference}{13.7 Warnings reference}}\label{depth-warnings-reference}\index{warnings!depth system}

All console lines follow the existing \texttt{"PAMPlayer: ..."}
convention and are deduplicated per \texttt{(name, warning-kind)} so a
character or prop redefined many times in a long scene doesn't spam
the console.

\begin{itemize}
\item
  \texttt{'<name>' has positive depth <d>; positive (foreground) depth
  is not supported in PAM 1.0.1; clamping to 0.}
\item
  \texttt{'<name>' has "rescale": true with depth 0; no-op (scale
  stays 1.0).}
\item
  \texttt{invalid depth <value> for '<name>'; using 0.0.} (non-numeric
  \texttt{"depth"})
\item
  \texttt{'<name>' is dual-registered with mismatched depth
  (cast=<a>, spawn\_prop=<b>); using the cast value.} (\S13.6)
\end{itemize}

\subsection{\texorpdfstring{13.8 Known limitations (out of scope for 1.0.1)}{13.8 Known limitations}}\label{depth-out-of-scope}\index{2.5D depth system!limitations}

Recorded here and in \texttt{BACK\_BURNER.md} as candidate later
items, not implemented in this pass:

\begin{itemize}
\item
  Automatic horizon $y$-offset per depth unit.
\item
  Per-bubble scaling (bubbles are always full-size, \S13.1's design
  choice, not a bug).
\item
  Depth on \texttt{spawn\_prop} animated over a curve other than
  linear.
\item
  Positive (foreground) depth.
\item
  Depth-aware \texttt{focus} dimming falloff.
\item
  A depth-only (no $x$ displacement) locomotion call (\S13.5).
\item
  \texttt{scene\_objects}/\texttt{"backdrop"} is not on the depth
  z\_index system at all --- it keeps its pre-existing
  \texttt{bring\_to\_back()} insertion-order mechanism (\S13.3).
\item
  An animated (not snap) transition when a mid-scene \texttt{cast}
  redefinition changes an already-live character's depth --- the new
  depth takes effect at the character's next \texttt{fade\_in}, same
  as every other cast-redefinable attribute.
\end{itemize}

\subsection{\texorpdfstring{13.9 Per-character SFX (backburner item 18)}{13.9 Per-character SFX}}\label{per-character-sfx}\index{sfx@\texttt{sfx}}\index{sfx\_gain@\texttt{sfx\_gain}}\index{sfx\_delay@\texttt{sfx\_delay}}\index{sfx\_trigger@\texttt{sfx\_trigger}}\index{sfx\_duration@\texttt{sfx\_duration}}\index{audio!per-character}\index{sound effects}

Builds on the item-7 in-scene audio plumbing (\texttt{\_SoundCues},
the \texttt{audio} preamble block, the \texttt{.m4a} auto-conversion
shim). Sound resolution for every step now walks a three-tier
priority ladder:

\begin{enumerate}
\item
  \textbf{Per-action inline} --- a \texttt{"sfx"} key on the step
  itself (\texttt{"sound"}, the item-7 key, is accepted as a legacy
  alias at this tier). Highest priority. An explicit falsy value
  (\texttt{"sfx": null}) suppresses \emph{all} tiers for this step.
\item
  \textbf{Cast-level \texttt{sfx} block} --- a per-character dict,
  keyed by action type, declared in the character's \texttt{cast}
  entry. Applies when the step names that character via \texttt{"who"}
  or \texttt{"prop"}. An explicit \texttt{null} value for an action
  type suppresses the scene default for that character's steps of
  that type.
\item
  \textbf{Scene \texttt{audio} defaults} --- item 7's preamble block,
  unchanged.
\end{enumerate}
...
{"action": "cast", "characters": {
  "pip": {
    "pose": "standing_front", "offset": [-2.5, 0, 0],
    "sfx": {
      "wave": "sfx/blip.wav",
      "say":  {"sfx": "sfx/chime.m4a", "sfx_gain": -3,
               "sfx_trigger": "end"}
    }
  }
}}
...
Each cast-block value is either a plain path string or an object with
\texttt{sfx} / \texttt{sfx\_gain} / \texttt{sfx\_delay} /
\texttt{sfx\_trigger} keys. The three parameter keys may also appear
inline on any step, where they override cast-tier values even when
the sound \emph{path} came from the cast tier:

\begin{itemize}
\item
  \texttt{sfx\_gain} (float, dB, default none) --- passed to Manim's
  \texttt{Scene.add\_sound(gain=...)}.
\item
  \texttt{sfx\_delay} (float, seconds, default \texttt{0}) --- delays
  the cue relative to its trigger point (maps to \texttt{add\_sound}'s
  \texttt{time\_offset}, which the Manim \emph{source} --- not its
  misleading docstring --- confirms is a playback delay:
  \texttt{time = self.time + time\_offset}).
\item
  \texttt{sfx\_trigger} (\texttt{"start"} default, or \texttt{"end"})
  --- \texttt{"start"} bakes the cue at the moment the step begins;
  \texttt{"end"} defers it until the step's animations have finished
  (implemented by parking the cue and flushing it when the renderer
  clock has advanced past the step --- at the top of the next loop
  iteration, or after the loop for a final step).
\item
  \texttt{sfx\_duration} (float, seconds, default none = play in
  full) --- cuts the cue to at most this length. A too-long cue
  (e.g.\ a 3-second \texttt{cue-bevers.m4a} capped at
  \texttt{"sfx\_duration": 1.0}) is trimmed by a single FFmpeg call
  that also handles format conversion in the same pass, with a 30 ms
  fade-out at the cut point so a hard mid-waveform cut doesn't click.
  Trimmed cues are cached per (file, duration) for the render, like
  the \texttt{.m4a} cache. Manim's own \texttt{add\_sound} API cannot
  express a trim (its kwargs reach \texttt{add\_audio\_segment}, not
  pydub's loader --- verified against the installed source), which is
  why this lives in PAM's FFmpeg shim. Invalid or non-positive values
  warn once and play the full cue; a missing \texttt{ffmpeg} binary
  warns once and plays the full cue rather than going silent.
\end{itemize}

\textbf{Semantics and caveats.}

\begin{itemize}
\item
  The cast \texttt{sfx} block participates in merge-on-redefine
  (item 19) with the same one-level merge as \texttt{"style"}: a
  mid-scene redefinition restating one action's sound keeps the
  character's other mappings.
\item
  Multi-target steps (\texttt{"who": [...]} or \texttt{"all"}) skip
  the cast tier --- firing one character's sound per target would
  stack N overlapping copies. Inline and scene tiers still apply.
\item
  \texttt{.m4a} files are auto-converted to \texttt{.wav} via FFmpeg
  and cached, exactly as for item 7 (this is design ``Option A'' ---
  everything through Manim's \texttt{add\_sound}; the sidecar-JSON +
  FFmpeg post-mixer ``Option B'' was not needed and was not built).
\item
  Parallel-block sub-steps resolve through the same ladder (the
  parallel handler already fires each sub-step's cue at block start);
  note \texttt{sfx\_trigger: "end"} inside a parallel block defers to
  the end of the \emph{whole block}, not the individual sub-action.
\end{itemize}

Verified with a spectral test, not just ``it rendered'': three
distinct-frequency generated tones (cast-tier 880 Hz, scene-default
220 Hz, \texttt{.m4a} 440 Hz) assigned across the tiers; decoding the
rendered video's audio track and detecting energy bursts found
\emph{exactly} the four expected events at the expected frequencies
and order --- cast-beats-default, default-fires-without-a-block,
inline-beats-default (with its 0.5 s delay), and the end-triggered
\texttt{.m4a} chime landing after its \texttt{say} completed --- and
\emph{no} event for the explicitly suppressed step.

\textbf{Scoring title and end credits (long cues).} Cues are not
limited to short effects: \texttt{add\_sound} mixes the \emph{whole}
file into the soundtrack from its cue point, so a multi-second music
bed attached to one step keeps playing across every step that
follows. Two placements cover the common cases:

\begin{itemize}
\item
  \textbf{Opening titles} --- the \texttt{title} block itself accepts
  an inline cue as of v0.9.23 (previously it was popped from the
  actions array before the sound resolver ever saw it, so an
  \texttt{"sfx"} key on it was silently ignored):

\begin{verbatim}
{"action": "title", "text": "TOO NICE TO DIE",
 "subtitle": "a PAM feature",
 "sfx": "credits_music.m4a", "sfx_duration": 8.5}
\end{verbatim}

  The cue is baked at the moment the title animation begins. Only the
  \emph{inline} tier applies to a title: there is no \texttt{"who"}
  (so no cast tier), and the scene \texttt{audio} defaults block sits
  \emph{after} the title in the actions array so has not been loaded
  yet --- order-dependent, like every other PAM preamble mechanism.
\item
  \textbf{End credits} --- there is no dedicated credits action;
  credit sequences are ordinary screenplay steps
  (\texttt{overlay\_caption} cards, \texttt{letter\_graph} props,
  etc.). Attach the cue to the \emph{first} step of the sequence (a
  \texttt{wait} step works if nothing else is at that spot) and it
  plays over everything that follows.
\end{itemize}

\textbf{Match the cue to the picture --- Manim does not truncate.}
Verified empirically: a 30-second cue over an 8.8-second scene
produces an \texttt{.mp4} whose audio stream is 30 seconds long
against an 8.8-second video stream, and players handle that mismatch
unpredictably (some stop at video end, some play on over a frozen
frame). Either cap the cue to the sequence length with
\texttt{sfx\_duration} (which also gives a clean fade-out at the cut),
or deliberately let it run long and shape the final soundtrack in
post with the item-8 \texttt{pam merge\_audio} CLI
(\texttt{--trim}/\texttt{--offset}/\texttt{--volume}/\texttt{--loop})
--- the right tool once you're mixing a real score rather than
placing cues.

Verified with a credits-style test scene
(\texttt{tests/depth/sfx\_credits\_capped.json}): an animated title
carrying a 30-second generated chord capped at 8.5 s, followed by
\texttt{overlay\_caption} credit cards and character animation ---
decoded audio shows continuous music from the title's first frame
through the end, with the audio stream (8.53 s) matched to the video
(8.80 s).

\subsection{\texorpdfstring{13.10 Structured warning registry (item 6, Option C)}{13.10 Structured warning registry}}\label{structured-warning-registry}\index{PAM\_WARNINGS\_JSON@\texttt{PAM\_WARNINGS\_JSON}}\index{warnings!structured registry}\index{warning registry}

Extension of the item-6 console-logging plumbing
(\texttt{PAM\_LOG} / \texttt{PAM\_WARNINGS\_ONLY}). Setting

\begin{verbatim}
PAM_WARNINGS_JSON=warnings.json PAM_SCRIPT=scene.json \
    manim -pql pam_player.py PAMPlayer
\end{verbatim}

records every PAM-prefixed console line emitted during the render as
a structured record and dumps the list as JSON at process exit:

\begin{verbatim}
[
  {"array_index": 1, "action_type": "cast",
   "message": "PAMPlayer: depth — 'carol' has positive depth 3; ..."},
  ...
]
\end{verbatim}

\texttt{array\_index} is the step's index in the \emph{screenplay
file} (the title-block pop that shifts the in-memory list is
compensated for); lines emitted outside any step --- module import,
preamble parsing before the main loop --- record \texttt{null} for
both fields. One documented ambiguity: when a sidecar
(item 1) is merged, indices refer to the \emph{merged} actions array,
which no longer matches the scene file line-for-line --- the merged
array is the more meaningful reference there, since sidecar blocks
can themselves produce warnings. Composes with
\texttt{PAM\_LOG}/\texttt{PAM\_WARNINGS\_ONLY}; all three can be
active at once.
...
\item
  Dog: \texttt{"dog\_standing"}, \texttt{"dog\_sitting"} (v0.9.22),
  \texttt{"dog\_trot\_a"}, \texttt{"dog\_trot\_b"}.
\end{itemize}
...
\subsection{5.1 Z-order for props and characters (v0.9.19)}\label{z-order-for-props}\index{z\_index@\texttt{z\_index}}\index{z-order}\index{layering|see{z-order}}\index{occlusion|see{z-order}}

Manim's compositing follows the order in which mobjects are added to the
scene. Without explicit overrides, the natural stack is:
...
\textbf{Footgun (pre-v0.9.19).} A \texttt{focus\_reset} re-plays
characters into the scene at restored opacity, which can push them above
props or bubbles added earlier. See §5.2. The same clobber can occur
during \texttt{spawn\_prop} calls that invoke \texttt{bring\_to\_back}.

\subsubsection{Explicit z-order: \texttt{z\_index}}\label{explicit-z-order}

As of v0.9.19, both props and characters accept a \texttt{z\_index} key.
Higher values render in front; lower values render behind. The default
Manim z-index is \texttt{0} for all mobjects, so any explicit value
competes only against other explicitly-set objects unless you set them
consistently.

\textbf{Props} (\texttt{spawn\_prop} or the manifest \texttt{props}
block):

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\{"action": "spawn\_prop", "prop": "back\_chair", "type": "chair",}
\NormalTok{ "x": -1.5, "z\_index": 1\}}

\NormalTok{\{"action": "spawn\_prop", "prop": "front\_table", "type": "table",}
\NormalTok{ "x": 0.0, "z\_index": 4\}}
\end{Highlighting}
\end{Shaded}

\textbf{Characters} (\texttt{fade\_in} step or the \texttt{cast} block):

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Option A: on the fade\_in step}
\NormalTok{\{"action": "fade\_in", "who": "chava", "offset": [0, 0, 0], "z\_index": 3\}}

\CommentTok{// Option B: in the cast block (multi-character scenes)}
\NormalTok{"cast": \{}
\NormalTok{  "chava": \{"figure\_type": "human", "offset": [0, 0, 0], "z\_index": 3\},}
\NormalTok{  "bosch": \{"figure\_type": "human", "offset": [2, 0, 0], "z\_index": 5\}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

\textbf{Resulting stack} for the example above (higher = closer to
camera):

\begin{center}
\begin{tabular}{cl}
\textbf{z\_index} & \textbf{object} \\
\hline
6 & speech bubbles (bring\_to\_front, always on top) \\
5 & bosch \\
4 & front\_table (occludes chava) \\
3 & chava (occludes back\_chair) \\
1 & back\_chair \\
0 & backdrop / building (default) \\
\end{tabular}
\end{center}

\subsubsection{Sticky z-order: surviving \texttt{bring\_to\_front}}\label{sticky-z-order}\index{\_reassert\_z\_indices@\texttt{\_reassert\_z\_indices}}\index{bring\_to\_front@\texttt{bring\_to\_front}!sticky z-order}

Internally, \texttt{pam\_player.py} calls \texttt{bring\_to\_front} or
\texttt{bring\_to\_back} in several places that would otherwise silently
clobber a carefully arranged stack:

\begin{itemize}
\tightlist
\item
  \texttt{scene\_objects} --- background dressing added with
  \texttt{bring\_to\_back}
\item
  \texttt{spawn\_prop} --- optional \texttt{bring\_to\_front}: true on a
  prop
\item
  \texttt{\_repair\_face\_overlays} --- called after every
  \texttt{focus} / \texttt{focus\_reset} to keep attached face PNGs on
  top of the stick-figure head dot
\end{itemize}

v0.9.19 introduces \texttt{\_reassert\_z\_indices()}, a private helper
that is called immediately after each of these operations. It re-applies
every character's stored \texttt{z\_index} to its \texttt{fig.group},
ensuring that focus animations and background-prop additions cannot
permanently bump a character out of its declared layer.

\textbf{Note:} \texttt{\_reassert\_z\_indices()} fires at discrete
moments, not continuously. During the \emph{frames} of a
\texttt{focus}/\texttt{focus\_reset} animation, the per-frame face
updater temporarily holds faces at the front; z-order is fully settled
once \texttt{\_repair\_face\_overlays} completes at the end of the play
call.

\subsubsection{The \texttt{z\_offset} alias}\label{z-offset-alias}

\texttt{z\_offset} is accepted as a screenplay alias for \texttt{z\_index}
on \texttt{spawn\_prop} steps and is treated identically. The canonical
key is \texttt{z\_index}; \texttt{z\_offset} is retained for backward
compatibility.

\subsubsection{Choosing z-index values}\label{choosing-z-index-values}

A simple convention that avoids conflicts:

\begin{center}
\begin{tabular}{cl}
\textbf{range} & \textbf{layer} \\
\hline
$\leq 0$ & deep background (sky, wall texture) \\
1--3 & background furniture (chair, bench behind character) \\
4--6 & characters \\
7--9 & foreground props (table, desk in front of character) \\
$\geq 10$ & always-on-top props (foreground bouquet, podium) \\
\end{tabular}
\end{center}

Speech bubbles are always placed above everything via
\texttt{bring\_to\_front} and are not affected by \texttt{z\_index}
values on characters or props.
